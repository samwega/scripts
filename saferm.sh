...
##
## Modified trash can path to that of pcmanfm, so get the original script if you're not using it >> https://github.com/lagerspetz/linux-stuff/blob/master/scripts/saferm.sh
...
# this is for pcmanfm's trash can
trash_desktops="$HOME/.local/share/Trash/files"
# if neither is running:
trash_fallback="$HOME/.local/share/Trash/files"
